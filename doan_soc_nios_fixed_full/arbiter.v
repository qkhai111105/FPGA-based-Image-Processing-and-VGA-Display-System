module arbiter (
	...
	always @(posedge clk or negedge rst_n) begin 
		if (rst_n == 1'b0) begin
         ...
         vga_addr_ptr  <= 18'd0;
			...
			avs_write_pulse <= 1'b0;
			avs_write_address <= 3'd0;
		end else begin 
			avs_readdatavalid_o <= avs_read_i;
			avs_write_pulse <= avs_write_i;
			avs_write_address <= avs_address_i;

			if (avs_read_i == 1'b1) begin
				case (avs_address_i)
					UART_ADDR_PTR: avs_readdata_o <= {14'd0, uart_addr_ptr};
					VGA_ADDR_PTR: avs_readdata_o <= {14'd0, vga_addr_ptr};
					EDGE_DETECT_WRITE_ADDR_PTR: avs_readdata_o <= {14'd0, edge_detect_write_addr_ptr};
					EDGE_DETECT_READ_ADDR_PTR: avs_readdata_o <= {14'd0, edge_detect_read_addr_ptr};
					INTERRUPT_STATUS_REGISTER: avs_readdata_o <= {30'd0, interrupt_status_reg};
					default: avs_readdata_o <= 32'd0;
				endcase
			end

			if (avs_write_i == 1'b1) begin
				case (avs_address_i)
					UART_ADDR_PTR: uart_addr_ptr <= avs_writedata_i[17:0];
					VGA_ADDR_PTR: vga_addr_ptr <= avs_writedata_i[17:0];
					EDGE_DETECT_WRITE_ADDR_PTR: edge_detect_write_addr_ptr <= avs_writedata_i[17:0];
					EDGE_DETECT_READ_ADDR_PTR: edge_detect_read_addr_ptr <= avs_writedata_i[17:0];
					PERFORM_EDGE_DETECTION_CMD: perform_edge_detect <= 1'b1;
					INTERRUPT_STATUS_REGISTER: interrupt_status_reg <= avs_writedata_i[1:0];
					default: begin
					end
				endcase
			end

			if (uart_wrapped == 1'b1) begin
				interrupt_status_reg[IRQ_UART_WRAPPED_IDX] <= 1'b1;
			end
			
			if (edge_detect_done == 1'b1) begin
				perform_edge_detect <= 1'b0;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (rst_n == 1'b0) begin
            ...
            edge_pixel_valid_o <= 1'b0;
            edge_fifo_rdreq_o <= 1'b0;
				vga_vsync_meta <= vga_vsync_i;
				vga_vsync_sync <= vga_vsync_meta;
            ...
            case (state)
                ST_IDLE: begin
                    ...
                    if (uart_req) begin
                     ...
                     // vga
                    else if (!vga_full_i && vga_vsync_sync) begin
                        state <= ST_VGA_RD_REQ;
                    end else begin
								state <= ST_IDLE;
						  end
                ...
                ST_VGA_RD_REQ: begin
                    // read sram
                    sram_addr_o <= vga_addr_ptr_active + vga_offset;
                    sram_read_o <= 1'b1;
            ...
            endcase
				...
				if (avs_write_pulse == 1'b1) begin
					case (avs_write_address)
				...
				if (vga_vsync_sync == 1'b0) begin
					vga_addr_ptr_active <= vga_addr_ptr;
					vga_offset <= 18'd0;
				end
        end
    end
